...
namespace QuantumConcepts.Formats.StereoLithography
{
    ...
    public class STLDocument : IEquatable<STLDocument>, IEnumerable<Facet>
    {
        ...
        {
            using (StreamWriter writer = new StreamWriter(stream, Encoding.ASCII, DefaultBufferSize))
            {
                //Write the header.
                writer.WriteLine(this.ToString());

                //Write each facet.
                this.Facets.ToList().ForEach(o => o.Write(writer));
        ...
        public static bool IsText(Stream stream)
        {
            ...
            return solid.Equals(header, StringComparison.OrdinalIgnoreCase);
        }
        ...
        public static STLDocument Read(Stream stream)
        {
            ...
            if (IsText(stream))
            {
                StreamReader reader = new StreamReader(stream, Encoding.ASCII, true, DefaultBufferSize);
                return Read(reader);
            }
            else
            {
        ...
        public bool Equals(STLDocument other)
        {
            return (this.Facets.Count == other.Facets.Count 
                && this.Facets.Except(other.Facets).Count() == 0);
        }
